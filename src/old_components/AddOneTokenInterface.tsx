...
export class AddOneTokenInterface extends React.Component<IProps, IState> {
  ...
  depositOneTokenAndStake() {
    const baseTokenNum = this.poolData.tokenIds.indexOf(
      this.poolData.baseTokenId
    );
    const baseTokenDecimals = this.poolData.tokenDecimals[baseTokenNum];

    const toStake = Math.round(
      Number(this.state.eggAmount) * baseTokenDecimals
    );
    ...
    globalSigner.signer
      ...
      .broadcast()
      .then((tx: any) => successMessage("You successfully performed deposit."))
  ...
  calculateDepositValue() {
    ...
    const baseTokenName = this.poolData.tokenNames[baseTokenNum];
    const baseTokenBalance = this.dicBalances[baseTokenName]
      ? this.dicBalances[baseTokenName].balance
      : 0;
    const baseTokenValue = this.dicBalances[baseTokenName]
      ? this.dicBalances[baseTokenName].value
      : 0;
    ...
    return (
      Math.round(
        (10000 / 98) *
          baseTokenValue *
          (Number(this.state.eggAmount) / baseTokenBalance)
      ) / 100
    );
  }
  ...
  render() {
    ...
    const userLiquidityValue =
      Math.floor(
        (this.calculateLiquidity() * userIndexTokens * 100) / totalIndexTokens
      ) / 100;
    ...
    return (
      <div className="addLiquidity">
        ...
        <div className="methodChoice">
          ...
          <div className="choice chosen second">With {baseTokenName} token</div>
        </div>
          ...
          <div className="poolDetails blockBody flex">
            <div className="poolData">
              ...
              <div className="poolValue">
                Total Index tokens:&nbsp;{totalIndexTokens}{" "}
                <span className="dollarValue">
                  (${this.calculateLiquidity()})
                </span>
              </div>
              <div className="poolValue">
                Your Index tokens:&nbsp;{userIndexTokens}{" "}
                <span className="dollarValue">(${userLiquidityValue})</span>
              </div>
            </div>
        ...
        {this.state.auth ? (
            ...
            <div className="compositionChoice">
              <div className="poolDetails blockBody">
                <div className="flex">
                  <div className="poolData flex">
                    ...
                    <div>
                      ...
                      <div className="poolValue dollarValue">
                  ...
                  <input
                    onChange={(e) => {
                      this.setState({ eggAmount: strToNum(e.target.value) });
                    }}
                    className="classicInput"
                    id="eggAmountInput"
                    defaultValue={baseTokenBalance}
                    type="text"
                  />
                </div>
                <div className="explanation flex">
                  <img
                    src={attentionImg}
                    className="attentionImg"
                    alt="attentione"
                  />
                  Your token will be automatically converted to other pool
                  tokens and provided as liquidity. Please pay attention that
                  value of your deposit can change if share token prices
                  fluctuate.
                  <br />
                  Because of slippage we don't recommend to invest more than
                  $1000 with one transaction!
                </div>
              </div>
              <div
                className={
                  this.state.allTokensHave ? "depositButtonContainer" : "hidden"
                }
              >
                <button
                  className="depositButton"
                  onClick={() => {
                    this.depositOneTokenAndStake();
                  }}
                >
                  Convert and deposit{" "}
                  <span className="">${this.calculateDepositValue()}</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="addLiquidity__login">
            <p className="addLiquidity__login-desc">
              Login to provide liquidity
            </p>

            <button
              onClick={() => {
                simulateMouseClick(document.getElementById("login")!);
              }}
              className="login-button button primary medium"
            >
              Login
            </button>
          </div>
