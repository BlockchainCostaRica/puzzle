...
export class AddLiquidityInterface extends React.Component<IProps, IState> {
    ...
    for (const name of this.poolData.tokenNames) {
      if (this.dicBalances[name] !== undefined) {
        ...
        const userTokenBalance =
          this.dicBalances[name].balance * this.dicBalances[name].decimals;
        const globalTokenBalance = poolState.get(
          "global_" + tokenId + "_balance"
        );
  ...
  calculateTokenToDeposit(tokenName: any) {
    ...
    const Dk =
      (((poolTokenAmount + minPIssued) / poolTokenAmount - 1) * Bk) /
      this.poolData.tokenDecimals[tokenNum];

    return (
      Math.floor(this.poolData.tokenDecimals[tokenNum] * Dk) /
      this.poolData.tokenDecimals[tokenNum]
    );
  }
  ...
  renderTokenDeposit(tokenName: any) {
    const tokenNum = this.poolData.tokenNames.indexOf(tokenName);
    const tokenDetails =
      this.dicBalances[tokenName] !== undefined
        ? this.dicBalances[tokenName]
        : { balance: 0, value: 0 };
    ...
    const tokenToDeposit =
      Math.floor(
        (this.poolData.tokenDecimals[tokenNum] *
          this.calculateTokenToDeposit(tokenName) *
          Number(this.state.percentage)) /
          100
      ) / this.poolData.tokenDecimals[tokenNum];
        ...
        <div className="tokenDeposit">
          <div className="tokenDeposit__lc">
            ...
            <div className="tokenDeposit__lc-desc">
              ...
              <div className="tokenDeposit__tokenWeight">
                <div>Share: </div>
                {tokenWeight}%
              </div>
            </div>
          ...
          <div className="tokenDeposit__rc">
            <div className="poolValue">
              {tokenToDeposit}{" "}
              <span className="dollarValue">/{tokenBalance}</span>
            </div>
          </div>
        </div>
      );
        ...
        <div className="tokenDeposit">
          ...
          <div className="tokenDeposit__rc">
            <div className="notEnough">
              <Link to="./">
                <span>buy {tokenToDeposit} tokens</span>
              </Link>
            </div>
          </div>
        </div>
          ...
          <div>
            <div className="amountChoice">
              ...
              <div className="blockBody">
                ...
                <div className="addAmountValue">
                  {this.state.percentage}%{" "}
                  <span className="dollarValue">
                    ($ {this.calculateDepositValue()})
                  </span>
                </div>
                <input
                  onInput={(e) => {
