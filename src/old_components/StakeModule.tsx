...
export class StakeModule extends React.Component<IProps, IState> {
  ...
  async componentDidMount() {
    const result = await axios.get(
      `${API_URL}/addresses/data/${stakingAddress}`
    );
    ...
    window.setInterval(() => {
      ...
      axios
        .get(`${API_URL}/transactions/address/${stakingAddress}/limit/20`)
        .then((result) => {
  ...
  claimRewards() {
    globalSigner.signer
      ...
      .broadcast()
      .then((tx: any) =>
        successMessage("You successfully claimed staking rewards.")
      )
  ...
  stake() {
    globalSigner.signer
      ...
      .broadcast()
      .then((tx: any) => successMessage("You successfully staked PUZZLE."))
      .catch((tx: any) => errorMessage(JSON.stringify(tx)));
  }
  ...
  unstake() {
    globalSigner.signer
      ...
      .broadcast()
      .then((tx: any) =>
        successMessage("You successfully removed PUZZLE from staking.")
      )
      .catch((tx: any) => errorMessage(JSON.stringify(tx)));
  }
  ...
  renderTransaction(tx: any) {
    if (
      tx.call !== undefined &&
      tx["stateChanges"]["invokes"][0] !== undefined
    ) {
      const amount =
        tx["stateChanges"]["invokes"][0]["payment"][0]["amount"] / 10 ** 6;
      const minutesAgo = this.state.height - tx.height;

      return (
        <div className="transactionData flex">
          <div className="flex">
            <img className="transactionIcon" src={payoutIcon} alt="" />
            <div className="columnFlex">
              <div className="transactionTitle">USDN Payout</div>
              <div className="transactionDate">
                {minutesAgo > 0 ? minutesAgo + " minutes ago" : "just now"}
              </div>
            </div>
          </div>
          <div className="">
            <div
              ...
              onClick={() => {
                window.open("https://wavesexplorer.com/tx/" + tx.id);
              }}
            >
              + {amount} USDN
            </div>
  ...
  render() {
    let userAddress,
      ...
      puzzleBalance;
    if (this.state.auth) {
      puzzleBalance = this.state.dicBalances["PUZZLE"]
        ? this.state.dicBalances["PUZZLE"].balance
        : 0;
      userAddress = localStorage.getItem("userAddress");
      stakedAmount = valueOrZero(this.state.data.get(userAddress + "_staked"));
      currentInterest = this.state.data.get(
        "global_lastCheck_DG2xFkPdDwKUoBkzGAhQtLpSGzfXLiCYPEzeKH2Ad24p_interest"
      );
      lastCheckInterest = valueOrZero(
        this.state.data.get(
          userAddress +
            "_lastCheck_DG2xFkPdDwKUoBkzGAhQtLpSGzfXLiCYPEzeKH2Ad24p_interest"
        )
      );
      toClaimAmount =
        Math.floor(
          (lastCheckInterest > 0
            ? stakedAmount * (currentInterest - lastCheckInterest)
            : 0) /
            10 ** 12
        ) /
        10 ** 6;
    ...
    return (
      <div className="addLiquidity stakeModule">
        <div className="head">
          <h1>PUZZLE Staking</h1>
          <p className="landing__desc">
            For every swap on the exchange, 0.4% fee is distributed as USDN to
            PUZZLE stakers proportionally to their staked balance.
          </p>
        </div>
                ...
                <div className="blockBody">
                  <div className="flex">
                    <div className="columnFlex">
                      ...
                      <div className="value">
                        {Math.floor(valueOrZero(puzzleBalance) * 10000) / 10000}{" "}
                        PUZZLE
                      </div>
                    </div>
                    <div className="columnFlex">
                      ...
                      <div className="name">
                        {valueOrZero(
                          this.state.data.get(userAddress + "_staked")
                        ) /
                          10 ** 8}{" "}
                        PUZZLE
                      </div>
                  ...
                  <div className="flex">
                    <button
                      onClick={() => this.setState({ formStatus: "stake" })}
                    >
                      Stake PUZZLE
                    </button>
                    <button
                      onClick={() => this.setState({ formStatus: "unstake" })}
                      className="withdraw"
                    >
                      Unstake
                    </button>
                ...
                {this.state.formStatus === "stake" ? (
                  <div className="stake-form swap-window">
                    <div className="">
                      ...
                      <div>
                        <button className="small" onClick={() => this.stake()}>
                          Stake PUZZLE
                        </button>
                      </div>
                    </div>
                  </div>
                ) : this.state.formStatus === "unstake" ? (
                  <div className="stake-form swap-window">
                    <div className="">
                      <div>
                        <input
                ...
                <div className="blockBody">
                  <div className="">
                    <div className="dollarValue">
                      Connect your wallet to see your stake balances
                    </div>
                    <button
                      onClick={() =>
          ...
          <div>
                    ...
                    <div className="columnFlex">
                      ...
                      <div className="big-value">
                        {valueOrZero(
                          this.state.data.get(
                            userAddress +
                              "_DG2xFkPdDwKUoBkzGAhQtLpSGzfXLiCYPEzeKH2Ad24p_claimed"
                          )
                        ) /
                          10 ** 6}{" "}
                        USDN
            ...
            <div className="history">
              <h2 className="preTitle">
                Transactions history (total protocol fees)
              </h2>
              <div className="blockBody">
