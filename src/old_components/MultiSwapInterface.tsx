...
export class MultiSwapInterface extends React.Component<IProps, IState> {
  ...
  async downloadState() {
    const data: Array<IContractStateKey> = (
      await axios.get(
        `${API_URL}/addresses/data/${this.poolData.contractAddress}`
      )
    ).data;
    ...
    return (
      Math.round(
        (10000 *
          coef *
          (this.state.data.get(
            "global_" + [this.poolData.tokenIds[t1]] + "_balance"
          ) /
            this.poolData.tokenShares[t1] /
            this.poolData.tokenDecimals[t1])) /
          (this.state.data.get(
            "global_" + [this.poolData.tokenIds[t2]] + "_balance"
          ) /
            this.poolData.tokenShares[t2] /
            this.poolData.tokenDecimals[t2])
      ) / 10000
    );
  }
  ...
  calculateAmountOut(coef: number) {
    ...
    const BalanceIn = this.state.data.get(
      "global_" + [this.poolData.tokenIds[tokenIn]] + "_balance"
    );
    const BalanceOut = this.state.data.get(
      "global_" + [this.poolData.tokenIds[tokenOut]] + "_balance"
    );

    const amountOut =
      (BalanceOut / this.poolData.tokenDecimals[tokenOut]) *
      (1 -
        (BalanceIn /
          (BalanceIn +
            this.poolData.tokenDecimals[tokenIn] * this.state.amountIn)) **
          (this.poolData.tokenShares[tokenIn] /
            this.poolData.tokenShares[tokenOut]));
    return (
      Math.floor(amountOut * this.poolData.tokenDecimals[tokenOut] * coef) /
      this.poolData.tokenDecimals[tokenOut]
    );
  }
  ...
  calculateLiquidity() {
    ...
    while (n < this.poolData.tokenNames.length) {
      sum +=
        (this.state.data.get(
          "global_" + [this.poolData.tokenIds[n]] + "_balance"
        ) *
          this.calculateCurrentPrice(usdnId, n, 1)) /
        this.poolData.tokenDecimals[n];
      n = n + 1;
    }
  ...
  getTokenInBalance() {
    const balanceData =
      this.dicBalances[this.poolData.tokenNames[this.state.tokenIn]];
  ...
  renderTokenChoice(tokenName: string, direction: string) {
    ...
    return (
      ...
      >
        <img
          className="tokenChoice-image"
          src={this.poolData.tokenLogos[n]}
          alt="token-choose"
        />
  ...
  render() {
    ...
    let cashbackAmount =
      ...
      this.calculateCurrentPrice(
        this.poolData.tokenNames.indexOf("USDN"),
        this.getTokenIn(),
        1
      );
    cashbackAmount =
      Math.floor((1000 * 0.96 * cashbackAmount) / puzzlePrice) / 1000;
          ...
          <>
              ...
              <div>
                <div className="comp">
                  <button
                    ...
                    className="infoIcon tokenData"
                    id="TokenIn"
                    type="button"
                  >
                    <img
                      className="tokenLogo"
                      src={this.poolData.tokenLogos[this.getTokenIn()]}
                      alt="tokenLogo"
                    />
                    <div className="tokenPair">
                      <div className="tokenName">
                        <span className="tokenName-text">You pay</span>
                        <span className="tokenName-name">
                          {this.poolData.tokenNames[this.getTokenIn()]}
                        </span>
                      </div>
                  ...
                  >
                    <PopoverHeader className="popover__header">
                      ...
                      <img
                        className="popover__header-icon"
                        onClick={() => {
                          this.closePopoverIn();
                        }}
                        src={closeIcon}
                    ...
                    <PopoverBody className="popover--body">
                      <div className="popover--body-input">
                        <img
                          src={searchIcon}
                          alt="search-icon"
                          className="textField__icon-left"
                        />
                        <input
                          type="text"
                          className="textField"
                          ...
                          onChange={(e) => {
                            this.changePopoverInSearchStr(e.target.value);
                          }}
                        />
                      </div>

                      {this.poolData.tokenNames
                        .filter(
                          (item: string) =>
                            item
                              .toUpperCase()
                              .indexOf(
                                this.state.popoverInSearchStr.toUpperCase()
                              ) > -1
                        )
                        .map((item: any) => this.renderTokenChoice(item, "in"))}
                    </PopoverBody>
                  ...
                  <div>
                    ...
                    >
                      max:&nbsp;{this.getTokenInBalance()}
                    </div>
                  </div>
                </div>
                <div
                  className={`change-button comp rate ${
                    this.state.changeButtonIsRotate
                      ? "change-button--rotate"
                      : ""
                  }`}
                  onClick={() => {
                    this.setState({
                      changeButtonIsRotate: !this.state.changeButtonIsRotate,
                    });
                    this.setState({
                      tokenOut: this.state.tokenIn,
                      tokenIn: this.state.tokenOut,
                    });
                  }}
                >
                  ...
                  <span className="rateText">
                    <span className="rateText">
                      1 {this.poolData.tokenNames[this.getTokenIn()]} ={" "}
                      {this.calculateCurrentPrice(
                        this.state.tokenOut,
                        this.getTokenIn(),
                        0.98
                      )}{" "}
                      {this.poolData.tokenNames[this.state.tokenOut]}
                    </span>
                    {/*<br/>*/}
                ...
                <div className="comp">
                  ...
                  >
                    <img
                      className="tokenLogo"
                      src={this.poolData.tokenLogos[this.state.tokenOut]}
                      alt="tokenLogo"
                    />
                  ...
                  <input
                    disabled={true}
                    placeholder={this.calculateAmountOut(0.98).toString()}
                  />
                    ...
                    <PopoverBody className="popover--body">
                      <div className="popover--body-input">
                        ...
                        <input
                          ...
                          placeholder="Search by name or ticker..."
                          onChange={(e) => {
                            this.changePopoverOutSearchStr(e.target.value);
                          }}
                        />
                      </div>
                ...
                <ModalWindow
                  ...
                  toReceive={
                    this.poolData.contractAddress ===
                    "3PFDgzu1UtswAkCMxqqQjbTeHaX4cMab8Kh"
                      ? 0
                      : Math.floor(
                          this.calculateAmountOut(0.95) *
                            this.poolData.tokenDecimals[this.state.tokenOut]
                        ) / this.poolData.tokenDecimals[this.state.tokenOut]
                  }
                  txData={{
                    pmt: {
                      ...
                      amount: Math.floor(
                        this.state.amountIn *
                          this.poolData.tokenDecimals[this.getTokenIn()]
                      ),
                    },
            ...
            <div className="pool-data">
              ...
              {this.poolData.layer2Address ? (
                <div>
                  <Link to={"/" + this.poolData.poolName + "/addLiquidity"}>
                    <button className="absolute button primary medium">
                      Add liquidity
                    </button>
                  </Link>
                </div>
              ) : (
                <img className="poolliq-image" src={puzzleBack} alt="poolliq" />
              )}
            </div>
            <div
              onClick={() => {
              ...
              className={`popover-overlay-helper ${
                this.state.popoverInIsOpen ? "active" : ""
              }`}
            />
            <div
              onClick={() => {
              ...
              className={`popover-overlay-helper ${
                this.state.popoverOutIsOpen ? "active" : ""
              }`}
            />
          </>
        )}
