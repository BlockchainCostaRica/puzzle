...
export class GlobalSigner {
  ...
  constructor() {
    this.authorizationMethod = localStorage.getItem(
      "authMethod"
    ) as String as "";
  ...
  async updateBalances() {
    ...
    const data: Array<any> = (
      await axios.get(
        `${API_URL}/assets/balance/${localStorage.getItem("userAddress")}`
      )
    ).data.balances;
    const wavesBalance = (
      await axios.get(
        `${API_URL}/addresses/balance/details/${localStorage.getItem(
          "userAddress"
        )}`
      )
    ).data.regular;
