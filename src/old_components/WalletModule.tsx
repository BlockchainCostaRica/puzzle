...
export function calculateTokenPrice(tokenInfo: any, states: any) {
  ...
  return (
    Math.round(
      (10000 *
        0.98 *
        (poolData.get("global_" + tokenId + "_balance") /
          poolData.get("static_" + tokenId + "_weight") /
          poolData.get("static_" + tokenId + "_scale"))) /
        (poolData.get("global_" + token2Id + "_balance") /
          poolData.get("static_" + token2Id + "_weight") /
          poolData.get("static_" + token2Id + "_scale"))
    ) / 10000
  );
}
...
export class WalletModule extends React.Component<IProps, IState> {
  ...
  componentDidMount() {
    if (
      ...
      window.localStorage.getItem("userBalances")
    ) {
        ...
        signer.getBalance().then((data: any) => {
          ...
          for (const asset of data) {
            if (appTokens.hasOwnProperty(asset.assetId)) {
              ...
              assetInfo["balance"] =
                Number(asset["amount"]) / assetInfo["decimals"];
              assetInfo["value"] =
                calculateTokenPrice(assetInfo, states) * assetInfo["balance"];
  ...
  renderTokenBalance(tokenId: any) {
    ...
    return (
      <div className="tokenBalance">
        ...
        <div>
          <span className="tokenBalance-value">
      ...
      <div className="wallet-module">
        ...
        >
          <span className="portfolio-value">
            ...
            {this.valueFormat(this.state.portfolioValue)}
        ...
        >
          ...
          <div className="popover--body-item">Copy address</div>
          ...
          <div
            className="popover--body-item popover--body-logout"
        ...
        <UncontrolledPopover
          className="wallet-popup infoPopup"
          trigger="focus"
          placement="bottom"
          target="PortfolioFocus"
        >
        ...
        <button
          onClick={() => {
            this.setState({ status: "authOpen" });
          }}
          id="login"
          className={
            this.state.status === "authed"
              ? "non-visible"
              : "open button primary medium"
          }
        >
          Connect wallet
        </button>
        ...
        <Modal
          toggle={() => {
            this.setState({ status: "nonauth" });
          }}
          isOpen={this.state.status === "authOpen"}
          className={"auth-window mt-5"}
        >
          <ModalHeader
            toggle={() => {
              this.setState({ status: "nonauth" });
          ...
          <ModalBody className="text-center">
            <button
              className="button secondary large wide button__icon"
              onClick={() => {
        ...
        <div
          ...
          className={`popover-overlay-helper ${
            this.state.addressModalIsOpen ? "active" : ""
          }`}
        ></div>
      </div>
    );
