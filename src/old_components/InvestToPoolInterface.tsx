...
export class InvestToPoolInterface extends React.Component<IProps, IState> {
  ...
  constructor(props: any) {
    ...
    const balances = JSON.parse(
      localStorage.getItem("userBalances") as String as "{}"
    );
  ...
  async downloadBalances() {
    const data: any = (
      await axios.get(
        `${API_URL}/assets/balance/${this.poolData.contractAddress}`
      )
    ).data;
    const dataMap = new Map();
  ...
  claimAllRewards() {
    globalSigner.signer
      ...
      .broadcast()
      .then((tx: any) => successMessage("You successfully claimed rewards."))
      .catch((tx: any) => errorMessage(JSON.stringify(tx)));
  }
  ...
  withdrawAll() {
    globalSigner.signer
          ...
          args: [
            {
              type: "integer",
              value: valueOrZero(
                this.state.poolState.get(
                  this.state.userAddress + "_indexStaked"
                )
              ),
            },
          ],
      ...
      .broadcast()
      .then((tx: any) => successMessage("Withdrawal went successful."))
      .catch((tx: any) => errorMessage(JSON.stringify(tx)));
  }
  ...
  calculateTokenAvailableReward(tokenName: string) {
    ...
    const balanceToDistr1 =
      this.state.poolBalances.get(tokenId) -
      this.state.poolState.get("global_" + tokenId + "_balance");
    const tokenBalanceLastCheck = Number(
      this.state.poolState.get("global_lastCheck_" + tokenId + "_earnings")
    );
    const newEarnings =
      Math.max(balanceToDistr1, tokenBalanceLastCheck) - tokenBalanceLastCheck;
    ...
    const currentInterest =
      totalStaked === 0
        ? 0
        : this.state.poolState.get(
            "global_lastCheck_" + tokenId + "_interest"
          ) +
          (newEarnings * decimals) / totalStaked;

    const lastCheckUserInterestVal = this.state.poolState.get(
      this.state.userAddress + "_lastCheck_" + tokenId + "_interest"
    );
    const lastCheckUserInterest = lastCheckUserInterestVal
      ? lastCheckUserInterestVal
      : 0;

    console.log(
      currentInterest,
      lastCheckUserInterest,
      currentInterest - lastCheckUserInterest
    );
    console.log(
      this.state.poolState.get(this.state.userAddress + "_indexStaked")
    );
    ...
    const tokenValue =
      ...
      this.calculateCurrentPrice(
        usdnId,
        this.poolData.tokenNames.indexOf(tokenName),
        1
      );
    ...
    if (!tokenBalance) {
      ...
      return {
        ...
        value: Math.floor(tokenValue * 100) / 100,
      };
    }
  }
    ...
    return (
      <div className="tokenDeposit">
        <div className="tokenDeposit__lc">
          <img
            className="tokenDeposit__smallLogo"
            src={this.poolData.tokenLogos[tokenNum]}
            alt=""
          />
        ...
        <div className="tokenDeposit__rc">
          <div className="tokenDeposit__poolValue">
            {tokenData.balance}{" "}
            <span className="dollarValue">($ {tokenData.value})</span>
          </div>
        </div>
      </div>
    );
  ...
  render() {
    const userClaimedValueVal = this.state.poolState.get(
      this.state.userAddress + "_claimedRewardValue"
    );
    const userClaimedValue = userClaimedValueVal ? userClaimedValueVal : 0;
    const userLastClaim = this.state.poolState.get(
      this.state.userAddress + "_lastClaim"
    );

    return (
      <div className="investToPool">
        <div className="column">
          ...
          <div className="portfolio block">
            ...
            <div className="blockBody">
                    ...
                    {valueOrZero(
                      Math.floor(
                        ((this.calculateLiquidity() *
                          ...
                          this.state.poolState.get("global_poolToken_amount")) *
                          100
                      ) / 100
                    )}
                  </div>
              ...
              <div className="flex">
                <button className="withdraw" onClick={() => this.withdrawAll()}>
                  Withdraw all
                </button>
                <button>
              ...
              <div className="subPreTitle columnFlex">
                <div>Total reward claimed:</div>
                <div className="dollarValue bigNumber">
                  ${userClaimedValue / 10 ** 6}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="rewards block">
            <div className="preTitle">
              Rewards to claim
              {userLastClaim ? (
                <div className="preTitle-after">
                  Unclaimed since {toDateString(userLastClaim)}
                </div>
              ) : (
                <div></div>
              )}
              ...
              <div className="blockBodyHead flex">
                <div className="columnFlex">
                  ...
                  <div className="mediumNumber">
                    $ {this.calculateTotalRewardValue()}
                  </div>
                </div>
                <div>
                  <button onClick={() => this.claimAllRewards()}>
                    Claim all
                  </button>
                </div>
              </div>
              <div>
                <div className="smallTitle">Reward composition</div>
                <div>
